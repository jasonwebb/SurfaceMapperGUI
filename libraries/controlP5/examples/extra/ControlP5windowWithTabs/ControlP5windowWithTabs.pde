...
* ControlP5 Tab
*
* shows how to add tabs to an extra window. do note that you need to update the
* window status after you have added a tab to an extra window with
*
* window.draw(); // assuming our extra ControlWindow is stored inside variable window
*
...
void setup() {
  ...
  ControlWindow window = cp5.addControlWindow("extraWindow");
  ...
  // update the window so that tabs are drawn properly
  window.draw();
}
